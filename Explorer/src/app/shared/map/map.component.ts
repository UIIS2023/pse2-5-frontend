...
export class MapComponent implements AfterViewInit{
  ...
  async setRouteForTourist(): Promise<number> {
    if (this.routingControl) {
      this.map.removeControl(this.routingControl);
    }
    ...
    this.routingControl = L.Routing.control({
      waypoints: this.getWaypoints(),
      router: L.routing.mapbox('pk.eyJ1IjoiYm9zaGtvNDIwIiwiYSI6ImNsbno0Y2xnZDEwenQyaXFtbWhoNGw3djEifQ.QgZuryjcj1pb-hGXF0ueRg', {profile: 'mapbox/walking'})
    }).addTo(this.map);
  
    // Wrap the callback in a Promise
    const routesFoundPromise = new Promise<number>((resolve) => {
      this.routingControl.on('routesfound', (e) => {
        var routes = e.routes;
        var summary = routes[0].summary;
        this.routeDistanceInMeters = summary.totalDistance;
  
        // Resolve the promise with the number value when the handler completes
        resolve(this.routeDistanceInMeters);
      });
    });
  
    // Wait for the promise to resolve before returning the number value
    return await routesFoundPromise;
  }

  getRouteDistanceFromFirstKeyPoint(firstLat: number, firstLng: number, touristLat: number, touristLng: number): Promise<number>{
    this.clearMarkers();
    this.addMarker(firstLat,firstLng);
    this.addMarker(touristLat,touristLng);

    return this.setRouteForTourist();
  }
}
